...
class ImageGenerator():
        ...
        """
        def generate_image_iterations(self,width,height,num_iters, class_num, output_file, input_file=None):
            self.initialize_net()
            net=self.net
                ...
                for i in range(num_iters):
                    for x in range(width-self.window_width):
                        for y in range(height-self.window_height):
                            ...
                            self.copy_array_section(0,0,x,y,self.window_width,self.window_height,new_arr,self.whole_image)
        ...
        def generate_fitting_image_iterations(self,num_iters, class_num , output_file, input_file):
            ...
            for i in range(num_iters):
                self.iterate_network(class_num,0,0)
                sys.stdout.flush()
                sys.stdout.write("\rIteration number : "+str(i))
        ...
        input_file is the start image that will be altered ; if None is specified then a noise image will be used
        """
        def generate_probable_image(self,width,height,threshold, class_num, output_file, input_file=None):
            probability=0.0
         
            self.initialize_net()
            net=self.net

            net.blobs[self.src_layer].data[:] = self.get_image_data(width,height)
